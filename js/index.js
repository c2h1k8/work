...
async function loadTabConfig() {
  ...
  return saved.slice().sort((a, b) => a.position - b.position);
}
...
function renderSettingsList(config) {
  ...
  list.innerHTML = "";
  const sorted = config.slice().sort((a, b) => a.position - b.position);
...
async function deleteTab(label) {
  ...
  // position を連番に詰め直す
  newConfig.slice().sort((a, b) => a.position - b.position)
