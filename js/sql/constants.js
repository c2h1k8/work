...
const TUNE_ITEMS = [
  ...
  { op: "NESTED LOOPS SEMI",
    category: "結合", level: "ok",
    desc: "IN / EXISTS を Nested Loops で処理するセミ結合。内側で最初にマッチした時点でループを打ち切るため効率的。" },
  { op: "NESTED LOOPS ANTI",
    category: "結合", level: "low",
    desc: "NOT IN / NOT EXISTS を Nested Loops で処理するアンチ結合。内側テーブルにインデックスがある場合に選択される。" },
  { op: "HASH JOIN",
    category: "結合", level: "low",
    desc: "小さい方のテーブルをハッシュテーブルに構築して大テーブルと等価結合。pga_aggregate_target の調整でディスクスピルを抑制できる。" },
  { op: "HASH JOIN OUTER",
    category: "結合", level: "low",
    desc: "Hash Join の外部結合版。大規模テーブルの LEFT OUTER JOIN で典型的に発生。" },
  { op: "HASH JOIN SEMI",
    category: "結合", level: "ok",
    desc: "IN / EXISTS をハッシュ方式で処理するセミ結合。内側でマッチが確認できた時点で外側の行を返す。" },
  { op: "HASH JOIN ANTI",
    category: "結合", level: "low",
    desc: "NOT IN / NOT EXISTS をハッシュ方式で処理するアンチ結合。大規模テーブルの NOT IN に有効。NULL 値の扱いに注意。" },
  { op: "MERGE JOIN",
    category: "結合", level: "low",
    desc: "両テーブルを結合キーでソートしてマージ（ソートマージ結合）。等価・不等価結合に対応。子ノードに SORT (JOIN) が表示される。既にソート済みであればコスト低。" },
  ...
  { op: "SORT AGGREGATE",
    ...
    desc: "GROUP BY なしの集計処理（COUNT(*) / SUM / MAX / MIN 等）。全件を集約して 1 行を返す。名前に SORT とあるが実際のソートは発生しない。" },
  { op: "HASH (GROUP BY)",
    category: "処理", level: "ok",
  ...
  { op: "SORT (ORDER BY)",
    ...
    desc: "行のソート処理。インデックスで ORDER BY を排除できないか条件を見直す。pga_aggregate_target を増やすとディスクスピルを抑制できる。" },
  { op: "SORT (GROUP BY)",
    category: "処理", level: "mid",
    desc: "集約のためのソート処理。USE_HASH_AGGREGATION ヒントで HASH (GROUP BY) への変換を検討する。" },
  ...
  { op: "BUFFER SORT",
    ...
    desc: "MERGE JOIN の内側入力を一時バッファに保存。pga_aggregate_target の調整でメモリ不足を改善できる。" },
  { op: "WINDOW SORT",
    category: "処理", level: "low",
    desc: "分析関数（ウィンドウ関数）のためのソート。PARTITION BY / ORDER BY の列にインデックスが利用できないか確認する。" },
  { op: "WINDOW BUFFER",
    category: "処理", level: "low",
    desc: "ROWS / RANGE 指定を伴う分析関数のバッファリング。pga_aggregate_target に収まるサイズか確認する。" },
