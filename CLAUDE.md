...
- `js/core/env.js` をクリップボード/通知を使うページで読み込む: `Env.type`（`'file'` / `'localhost'` / `'tauri'`）/ `Env.isTauri` / `Env.isLocalhost` / `Env.isFile` — 実行環境検出。`file://` / localhost / Tauri デスクトップアプリの3形態を判別。iframe 内では `window.parent.__TAURI__` にもフォールバックして検出
...
- `js/core/file_saver.js` をエクスポート機能があるページで読み込む（`env.js` に依存）: `FileSaver.save(content, defaultName, opts?)` → Promise\<boolean\>。Tauri では `tauri-plugin-dialog` の保存ダイアログ + `tauri-plugin-fs` でファイル書き込み（iframe 内では親フレームの `__TAURI__` にフォールバック）、それ以外は Blob + `<a>` クリックでダウンロード。**Blob + `<a>.click()` によるダウンロードを直接書かない。必ず `FileSaver.save` を使うこと**。使用ページ: `index.html` / `todo.html` / `note.html` / `snippet.html` / `sql.html` / `dashboard.html` / `wbs.html` / `timer.html`
- `js/core/notify.js` を通知機能があるページで読み込む（`env.js` に依存）: `Notify.send(title, body, opts?)` / `Notify.requestPermission()` / `Notify.getPermission()` — 環境対応通知。`file://` では `'unsupported'`、localhost では Web Notifications API、Tauri ではネイティブ通知（iframe 内では親フレームの `__TAURI__` にフォールバック）。**`Notification` API を直接使わない。必ず `Notify` を使うこと**。使用ページ: `timer.html`
